...
int dfs(int i, int j, int dirx_prev, int diry_prev, const vector<vector<int>>& topoMap) {
    int r = topoMap.size();
    int c = topoMap[0].size();
    
    //Commented this one part out to get all trails(ie non-unique trails)
    // if (topoMap[i][j] == 9){
    ...
    for (int k = 0; k < 4; k++) {
        ...
        if (isValid(nx, ny, r, c) && topoMap[nx][ny] == topoMap[i][j] + 1) {
            ...
            branches++;
            int dfsRes = dfs(nx, ny, dirx[k], diry[k], topoMap);
            paths += dfsRes;
        }
    }
